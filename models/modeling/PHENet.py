...
class DifferenceFeatureExtractor(nn.Module):
    ...
    def forward(self, img1, img2, height_diff):
        ...
        feat1 = img1 * (1+w)
        feat2 = img2 * (1+w)
        return self.fusion_conv(torch.cat([feat1, feat2, torch.abs(feat1 - feat2)], dim=1))
...
class PHENet(nn.Module):
    ...
    def forward(self, x1, x2,h1,h2):
        ...
        h1 = self._normalize_height(h1)
        h2 = self._normalize_height(h2)
        if self.training:
    ...
    def _normalize_height(height):
        ...
        return (height - minimum) / (maximum - minimum).clamp_min(1e-6)
    def freeze_bn(self):
