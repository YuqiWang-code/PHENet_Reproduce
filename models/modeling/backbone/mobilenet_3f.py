...
class MobileNetV2(nn.Module):
    def __init__(self, output_stride=8, BatchNorm=None, width_mult=1., pretrained=True, in_channels=3):
        ...
        if pretrained:
            print("pretained model load ...")
            self._load_pretrained_model()

        self.low_level_features = self.features[0:4]
        self.mid_level_features = self.features[4:7]
    ...
    def _load_pretrained_model(self):
        checkpoint_path = Path(__file__).with_name('mobilenet_v2-6a65762b.pth')
        if not checkpoint_path.is_file():
            raise FileNotFoundError(f"MobileNetV2 checkpoint not found: {checkpoint_path}")
        pretrain_dict = torch.load(checkpoint_path, map_location='cpu')
...
if __name__ == "__main__":
    ...
    low_level_feat, mid_level_feat, high_level_feat = model(input)
    print(low_level_feat.size())
    print(mid_level_feat.size())
