...
CHECKPOINT="/storage/yqwang/PHENet/saved_models/Run1/LEVIR-CD+foggy/best_F1=0.6525.pth"
...
RESULT_ROOT="/storage/yqwang/PHENet/generalization/Run1/LEVIR-CD+foggy_best_F1_0.6525/HH"
...
echo "=== PHENet zero-shot cross-domain generalization: LEVIR-CD+foggy -> HH ==="
...
for subset in "${SUBSETS[@]}"; do
    ...
    echo
    echo "=== Evaluating LEVIR-CD+foggy -> ${subset} ==="
    ...
    echo "[PASS] LEVIR-CD+foggy -> ${subset}: ${output_log}"
done

echo
echo "[PASS] LEVIR-CD+foggy -> all three HH subsets completed."
